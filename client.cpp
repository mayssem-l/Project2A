...
bool Client::ajouter() {
    QSqlQuery query;
    query.prepare("INSERT INTO CLIENT (ID_C, NOM_C, FIRST_NAME_C, AGE_C, ADRESSE_C, REGION_C, E_MAIL_C, TEL_C) "
...
QSqlQueryModel* Client::afficher() {
    ...
    model->setQuery("SELECT * FROM YASSINE.CLIENT");
...
bool Client::modifier() {
    QSqlQuery query;
    query.prepare("UPDATE CLIENT SET NOM_C = :name, FIRST_NAME_C = :first_name, AGE_C = TO_DATE(:age, 'YYYY-MM-DD'), "
...
bool Client::supprimer(int clientId) {
    QSqlQuery query;
    query.prepare("DELETE FROM YASSINE.CLIENT WHERE ID_C = :id");
...
QSqlQueryModel* Client::search(int clientId) {
    ...
    query.prepare("SELECT * FROM YASSINE.CLIENT WHERE ID_C = :id");
    query.bindValue(":id", clientId);
